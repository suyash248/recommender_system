...
def new_prod_meta():
    ...
    return {
        ...
        "reviews": {},
        "similar": {},
        "categories": {"count": 0, "details": OrderedDict()}
    }
...
def parse_create_schema_from_meta_file_partially(meta_file_name='amazon-meta.txt.gz',
    ...
    q = """
            ...
            WITH {categories_details_list} as cat_details_list_with, prod1
            UNWIND cat_details_list_with as cat_details
            MERGE (cat1:`Category`{id: cat_details.id, name: cat_details.name})
            MERGE (prod1) -[:BELONGS_TO]-> (cat1)
    ...
    for chunk in reader:
        for lines in chunk.values:
            ...
            if line.startswith("Id:"):
                if len(prod_meta) > 0:
                    # cat_details = prod_meta["categories"].get("details", OrderedDict())
                    # cat_id = cat_name = 'NA'
                    ...
                    cat_details_raw = prod_meta["categories"].get("details", OrderedDict())
                    cat_details_list = []
                    cindex = 0
                    for cname, cid in cat_details_raw.items():
                        cat_details_list.append({
                            "id": cid,
                            "name": cname,
                            "index": cindex
                        })
                    ...
                    params = {
                        ...
                        "categories_details_list": cat_details_list,
                ...
                # prod_meta['categories'] = prod_meta.get('categories', {})
                # prod_meta['categories']['details'] = prod_meta['categories'].get("details", set())
                ...
                for cat_name_id in categories_name_ids:
                    ...
                    if cat_name and cat_id:
                        if cat_name not in prod_meta['categories']['details']:
                            prod_meta['categories']['details'][cat_name] = cat_id
...
if __name__ == '__main__':
    ...
    parse_create_schema_from_meta_file_partially(batch_size=100)
